...
public final class DeviceNameUtil {
  public static String encryptDeviceName(String plainTextDeviceName, ECPrivateKey identityKey) {
    ...
    ECKeyPair ephemeralKeyPair = ECKeyPair.generate();

    try {
      Mac    mac    = Mac.getInstance("HmacSHA256");
      Cipher cipher = Cipher.getInstance("AES/CTR/NoPadding");

      byte[] masterSecret = identityKey.calculateAgreement(ephemeralKeyPair.getPublicKey());
  ...
  public static String decryptDeviceName(String encryptedDeviceName, ECPrivateKey identityKey) throws IOException {
    ...
    try {
      ECPublicKey ephemeralPublic = new ECPublicKey(ephemeralPublicBytes);
      byte[]      masterSecret    = identityKey.calculateAgreement(ephemeralPublic);
