...
import org.whispersystems.signalservice.api.messages.SignalServiceDataMessage.AdminDelete
...
import org.whispersystems.signalservice.api.messages.SignalServiceDataMessage.PinnedMessage
import org.whispersystems.signalservice.api.messages.SignalServiceDataMessage.PollCreate
import org.whispersystems.signalservice.api.messages.SignalServiceDataMessage.PollTerminate
import org.whispersystems.signalservice.api.messages.SignalServiceDataMessage.PollVote
...
import org.whispersystems.signalservice.api.messages.SignalServiceDataMessage.UnpinnedMessage
...
class SignalServiceContent {
  ...
  companion object {
    ...
    private fun createSignalServiceDataMessage(
      ...
      val pollCreate = createPollCreate(content.pollCreate)
      val pollVote = createPollVote(content.pollVote)
      val pollTerminate = createPollTerminate(content.pollTerminate)
      ...
      val pinnedMessage = createPinnedMessage(content.pinMessage)
      val unpinnedMessage = createUnpinnedMessage(content.unpinMessage)
      val adminDelete = createAdminDelete(content.adminDelete)
      return newBuilder()
        ...
        .withExpireTimerVersion(content.expireTimerVersion ?: 1)
        ...
        .withPollVote(pollVote)
        .withPollTerminate(pollTerminate)
        .withPinnedMessage(pinnedMessage)
        .withUnpinnedMessage(unpinnedMessage)
        .withAdminDelete(adminDelete)
        .build()
    }
    ...
    private fun createSynchronizeMessage(
      ...
      if (content.blocked != null) {
        val blockedE164s = if (content.blocked.blockedE164s.isNotEmpty()) {
          content.blocked.blockedE164s.mapNotNull { blockedE164 ->
            blockedE164.e164?.let { BlockedListMessage.Individual(null, it, blockedE164.timestamp ?: 0) }
          }
        } else {
          content.blocked.numbers.map { BlockedListMessage.Individual(null, it, 0) }
        }
        val blockedAcis = if (content.blocked.blockedAcis.isNotEmpty()) {
          content.blocked.blockedAcis.mapNotNull { blockedAci ->
            ACI.parseOrNull(blockedAci.aciBinary)?.let { BlockedListMessage.Individual(it, null, blockedAci.timestamp ?: 0) }
          }
        } else if (content.blocked.acisBinary.isNotEmpty()) {
          content.blocked.acisBinary.mapNotNull { aci ->
            ACI.parseOrNull(aci)?.let { BlockedListMessage.Individual(it, null, 0) }
          }
        } else {
          content.blocked.acis.mapNotNull { aci ->
            ACI.parseOrNull(aci)?.let { BlockedListMessage.Individual(it, null, 0) }
          }
        }
        val blockedGroups = if (content.blocked.blockedGroups.isNotEmpty()) {
          content.blocked.blockedGroups.mapNotNull { blockedGroup ->
            blockedGroup.groupId?.toByteArray()?.let { BlockedListMessage.Group(it, blockedGroup.timestamp ?: 0) }
          }
        } else {
          content.blocked.groupIds.map { BlockedListMessage.Group(it.toByteArray(), 0) }
        }
        return SignalServiceSyncMessage.forBlocked(BlockedListMessage(blockedE164s + blockedAcis, blockedGroups))
      }
      ...
      if (content.keys != null) {
        val mediaRootBackupKey = content.keys.mediaRootBackupKey?.toByteArray()
        val accountEntropyPool = content.keys.accountEntropyPool
        return SignalServiceSyncMessage.forKeys(
          KeysMessage(
            null,
            accountEntropyPool?.let {
              AccountEntropyPool(
                it
              )
            },
            mediaRootBackupKey?.let {
              MediaRootBackupKey(
                it
              )
            }
      ...
      if (content.deviceNameChange != null) {
        return SignalServiceSyncMessage.forDeviceNameChange(content.deviceNameChange)
      }
    ...
    @Throws(InvalidMessageStructureException::class)
    private fun createPollCreate(content: DataMessage.PollCreate?): PollCreate? {
      if (content?.question == null || content.allowMultiple == null) {
        return null
      }
      return PollCreate(content.question, content.allowMultiple, content.options)
    }

    @Throws(InvalidMessageStructureException::class)
    private fun createPollVote(content: DataMessage.PollVote?): PollVote? {
      if (content?.targetAuthorAciBinary == null || content.targetSentTimestamp == null || content.voteCount == null) {
        return null
      }
      return PollVote(
        ACI.parseOrThrow(content.targetAuthorAciBinary),
        content.targetSentTimestamp,
        content.optionIndexes,
        content.voteCount
      )
    }

    @Throws(InvalidMessageStructureException::class)
    private fun createPollTerminate(content: DataMessage.PollTerminate?): PollTerminate? {
      if (content?.targetSentTimestamp == null) {
        return null
      }
      return PollTerminate(content.targetSentTimestamp)
    }

    @Throws(InvalidMessageStructureException::class)
    private fun createPinnedMessage(content: DataMessage.PinMessage?): PinnedMessage? {
      if (content?.targetAuthorAciBinary == null || content.targetSentTimestamp == null) {
        return null
      }
      return PinnedMessage(
        ACI.parseOrThrow(content.targetAuthorAciBinary),
        content.targetSentTimestamp,
        content.pinDurationSeconds,
        content.pinDurationForever
      )
    }

    @Throws(InvalidMessageStructureException::class)
    private fun createUnpinnedMessage(content: DataMessage.UnpinMessage?): UnpinnedMessage? {
      if (content?.targetAuthorAciBinary == null || content.targetSentTimestamp == null) {
        return null
      }
    ...
    @Throws(InvalidMessageStructureException::class)
    private fun createAdminDelete(content: DataMessage.AdminDelete?): AdminDelete? {
      if (content?.targetAuthorAciBinary == null || content.targetSentTimestamp == null) {
        return null
      }
      return AdminDelete(
        ...
        val builder = SharedContact.newBuilder()
          .setName(
            SharedContact.Name.newBuilder()
              ...
              .setNickname(contact.name?.nickname)
              .build()
          )
      ...
      if (content.group.type !== GroupContext.Type.DELIVER) {
        ...
        if (pointer?.key != null) {
          avatar = SignalServiceAttachmentPointer(
            ...
            UuidUtil.parseOrNull(pointer.clientUuid?.toByteArray())
          )
        }
