  ...
  companion object {
    ...
    private fun createSynchronizeMessage(
      ...
      if (content.blocked != null) {
        val numbers = content.blocked.numbers.map { BlockedListMessage.Individual(null, it) }
        val uuids = content.blocked.acis.mapNotNull { ACI.parseOrNull(it) }.map { BlockedListMessage.Individual(it, null) }
        val addresses = numbers + uuids
